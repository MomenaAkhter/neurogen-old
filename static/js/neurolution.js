    ...
    for (var i in range(len(nn.weights))) {
        ...
        for (var j in range(size_y)) {
            ...
            for (var k in range(size_x)) {
                ...
                var random_variation = nn.weights[i][j][k] * amount * Math.round(random(-1, 1));
...
function breed_generation(model) {
    ...
    var selection_count = model.population_size * .1;
...
function best_fit_select(model, quantity) {
    ...
    var output = "Generation " + (model.current_generation_index + 1) + ": " + len(model.generations);
    var average = 0;
    individuals.cars.forEach(function (car) {
        output += car.fitness + " ";
        average += car.fitness;
    });
...
function update(time, delta) {
    ...
    var lg = last_generation(sm);
    // get rid of really slow cars after first 2 seconds
    if ((Date.now() - lg.time) / 1000 > 2) {
        lg.cars.forEach(function (car) {
            if (car.speed < 0.2 && !car.destroyed && car.user_data != (sm.human_controlled_car === undefined ? {} : sm.human_controlled_car.user_data))
                mark_car_for_destruction(car);
        });
    }
    ...
    if (number_of_cars_alive == 0) {
        ...
        lg.cars.forEach(function (car) {
            add_car_to_world(sm, car);
            add_car_to_scene(sm, car);
            // output += (Date.now() - car.creation_timestamp) + " ";
        });
        // console.log(output);
